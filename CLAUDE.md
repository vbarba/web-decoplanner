...
**The shared engine contract is the spine of the codebase.** Both engines expose `<NS>.plan(input) -> result` with identical shapes, so the UI treats them interchangeably. When changing one engine's behavior, the **other engine and the UI must stay consistent with the same contract** — the engines share the same `result` shape and the same schedule conventions for the canonical cases (stop ladder, gas auto-switch at MOD, `lastStopDepth` handling, CNS/OTU, gas-usage formula, profile sampling). There are **two documented, intentional differences** that are *not* mirrored: NDL semantics (ZHL reports at the final/shallowest depth, VPM-B at the controlling/deepest depth) and whether an already-clear intermediate stop rung is held (ZHL skips it, VPM-B holds it) — see `docs/DECISIONS.md` before "fixing" either. The canonical description of `input` and `result` is the block comment near the top of `js/engine/zhl16.js`; `result` carries `ok/errors/warnings`, `table` (row `phase`: `desc|level|asc|switch|stop`), `stops`, `gasUsage`, `oxygen` (cns/otu), `profile`, `ceilingProfile`, `finalTissues`.
