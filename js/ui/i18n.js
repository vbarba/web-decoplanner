...
(function () {
  ...
  var DICT = {
    en: {
      ...
      'tip.matrix': 'Decompression-stop times over a range of bottom times. One sub-table per depth (planned ±3 m); columns are bottom times, rows are stop depths.',
      'tip.mtxStart': 'Shortest bottom time in the range (min).',
      'tip.mtxEnd': 'Longest bottom time in the range (min).',
      'tip.mtxStep': 'Step between bottom times (min).',
      'tip.copyMatrix': 'Copy the table as tab-separated values (paste into Excel).',
      ...
      'mtx.minutes': 'min',
      'mtx.stopDepth': 'STOP',
      'mtx.depthAt': 'Bottom time at {d}',
      'mtx.totalDeco': 'TOTAL DECO',
      'mtx.grp.deep': 'Deep (≥39 m)',
      'mtx.grp.deepInt': 'Deep intermediate (24–36 m)',
      'mtx.grp.inter': 'Intermediate (9–21 m)',
      'mtx.grp.shallow': 'Shallow (3+6 m)',
      'mtx.empty': 'No bottom times in range — set START, END and INTERVAL above.',
      'mtx.legend': 'Columns = bottom times (min); rows = decompression-stop times (min) at each stop depth. One block per depth (planned ±3 m). Grouped rows sum the stops in each band; TOTAL DECO is all stop time.',
    ...
    es: {
      ...
      'box.matrix': 'Tabla de tiempos de deco',
      ...
      'tip.matrix': 'Tiempos de parada de descompresión para un rango de tiempos de fondo. Una subtabla por profundidad (planificada ±3 m); las columnas son tiempos de fondo, las filas son profundidades de parada.',
      'tip.mtxStart': 'Tiempo de fondo más corto del rango (min).',
      'tip.mtxEnd': 'Tiempo de fondo más largo del rango (min).',
      'tip.mtxStep': 'Paso entre tiempos de fondo (min).',
      'tip.copyMatrix': 'Copiar la tabla como valores separados por tabuladores (pegar en Excel).',
      ...
      'mtx.step': 'INTERVALO',
      'mtx.minutes': 'min',
      'mtx.stopDepth': 'PARADA',
      'mtx.depthAt': 'Tiempo de fondo a {d}',
      'mtx.totalDeco': 'DECO TOTAL',
      'mtx.grp.deep': 'Profunda (≥39 m)',
      'mtx.grp.deepInt': 'Intermedia profunda (24–36 m)',
      'mtx.grp.inter': 'Intermedia (9–21 m)',
      'mtx.grp.shallow': 'Somera (3+6 m)',
      'mtx.empty': 'No hay tiempos de fondo en el rango: define INICIO, FIN e INTERVALO arriba.',
      'mtx.legend': 'Columnas = tiempos de fondo (min); filas = tiempos de parada de descompresión (min) en cada profundidad. Un bloque por profundidad (planificada ±3 m). Las filas agrupadas suman las paradas de cada banda; DECO TOTAL es todo el tiempo de parada.',
    ...
    fr: {
      ...
      'box.matrix': 'Table des temps de déco',
      ...
      'tip.matrix': 'Temps de paliers de décompression sur une plage de temps au fond. Une sous-table par profondeur (planifiée ±3 m) ; les colonnes sont les temps au fond, les lignes les profondeurs de palier.',
      'tip.mtxStart': 'Temps au fond le plus court de la plage (min).',
      'tip.mtxEnd': 'Temps au fond le plus long de la plage (min).',
      'tip.mtxStep': 'Pas entre les temps au fond (min).',
      'tip.copyMatrix': 'Copier la table en valeurs séparées par des tabulations (coller dans Excel).',
      ...
      'mtx.step': 'INTERVALLE',
      'mtx.minutes': 'min',
      'mtx.stopDepth': 'PALIER',
      'mtx.depthAt': 'Temps au fond à {d}',
      'mtx.totalDeco': 'DÉCO TOTALE',
      'mtx.grp.deep': 'Profond (≥39 m)',
      'mtx.grp.deepInt': 'Intermédiaire profond (24–36 m)',
      'mtx.grp.inter': 'Intermédiaire (9–21 m)',
      'mtx.grp.shallow': 'Peu profond (3+6 m)',
      'mtx.empty': 'Aucun temps au fond dans la plage — définissez DÉBUT, FIN et INTERVALLE ci-dessus.',
      'mtx.legend': 'Colonnes = temps au fond (min) ; lignes = temps de palier de décompression (min) à chaque profondeur. Un bloc par profondeur (planifiée ±3 m). Les lignes groupées somment les paliers de chaque tranche ; DÉCO TOTALE est tout le temps de palier.',
    ...
    de: {
      ...
      'box.matrix': 'Deko-Zeiten-Tabelle',
      ...
      'tip.matrix': 'Dekompressions-Stoppzeiten über einen Bereich von Grundzeiten. Eine Untertabelle pro Tiefe (geplant ±3 m); Spalten sind Grundzeiten, Zeilen sind Stopptiefen.',
      'tip.mtxStart': 'Kürzeste Grundzeit im Bereich (min).',
      'tip.mtxEnd': 'Längste Grundzeit im Bereich (min).',
      'tip.mtxStep': 'Schritt zwischen den Grundzeiten (min).',
      'tip.copyMatrix': 'Tabelle als tabulatorgetrennte Werte kopieren (in Excel einfügen).',
      ...
      'mtx.minutes': 'min',
      'mtx.stopDepth': 'STOPP',
      'mtx.depthAt': 'Grundzeit bei {d}',
      'mtx.totalDeco': 'DEKO GESAMT',
      'mtx.grp.deep': 'Tief (≥39 m)',
      'mtx.grp.deepInt': 'Tief-mittel (24–36 m)',
      'mtx.grp.inter': 'Mittel (9–21 m)',
      'mtx.grp.shallow': 'Flach (3+6 m)',
      'mtx.empty': 'Keine Grundzeiten im Bereich — START, ENDE und INTERVALL oben festlegen.',
      'mtx.legend': 'Spalten = Grundzeiten (min); Zeilen = Dekompressions-Stoppzeiten (min) je Stopptiefe. Ein Block pro Tiefe (geplant ±3 m). Gruppenzeilen summieren die Stopps je Band; DEKO GESAMT ist die gesamte Stoppzeit.',
    ...
    zh: {
      ...
      'tip.matrix': '一系列底时下的减压停留时间。每个深度一张子表（计划 ±3 m）；列为底时，行为停留深度。',
      'tip.mtxStart': '范围内最短的底时（分钟）。',
      'tip.mtxEnd': '范围内最长的底时（分钟）。',
      'tip.mtxStep': '底时之间的步长（分钟）。',
      'tip.copyMatrix': '以制表符分隔的形式复制表格（可粘贴到 Excel）。',
      ...
      'mtx.totalDeco': '减压总计',
      'mtx.grp.deep': '深（≥39 m）',
      'mtx.grp.deepInt': '深-中（24–36 m）',
      'mtx.grp.inter': '中（9–21 m）',
      'mtx.grp.shallow': '浅（3+6 m）',
      'mtx.empty': '范围内无底时——请在上方设置起始、结束和间隔。',
      'mtx.legend': '列＝底时（分钟）；行＝各停留深度的减压停留时间（分钟）。每个深度一块（计划 ±3 m）。分组行汇总各档停留；减压总计为全部停留时间。',
